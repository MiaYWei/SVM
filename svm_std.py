...
classifier = SVC(kernel='linear', class_weight='balanced', probability=True, shrinking=False, cache_size=10000, verbose=True, random_state=42)
...
prData = {'Pr':svm_precision, 'Re':svm_recall}
df = pd.DataFrame(prData)
df.sort_index(ascending=True)
Pr_mean = df[round(df['Re'],2)==0.50].iloc[:,0].mean()
Pr_std = df[round(df['Re'],2)==0.50].iloc[:,0].std()
Pr_max = df[round(df['Re'],2)==0.50].iloc[-1,0]
print('Max Pr@Re50 :%.5f (Std = %.5f)'%(Pr_max, Pr_std))
